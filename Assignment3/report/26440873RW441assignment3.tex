...
\subsection{\textbf{RNN Architecture}}

The RNN investigated in this study were implemented using the PyTorch API. Pytorch is a highly customizable open-source deep learning framework. Each RNN type was defined as a custom \texttt{nn.Module} class, with a forward pass implementation tailored to the recurrence formula of the different architectures.

\begin{itemize}
    \item \textbf{Elman RNN}: Implemented using PyTorch's built-in nn.RNN layer, which is a plain Elman RNN.
    \item \textbf{Jordan RNN}: Implemented from scratch to include feedback from previous predicted output into the current hidden layer.
    \item \textbf{Multi-RNN}: Implemented from scratch as a hybrid of the Elman and Jordan architectures, combining hidden-to-hidden and output-to-hidden recurrence.
\end{itemize}

Each model implementation was parameterized, which allows users to choose hidden layer size, activation function, and output type (sequence-to-one vs. sequence-to-sequence). The implementations also included gradient clipping to prevent the exploding/vanishing gradient problem.
...
\begin{itemize}
    ...
    \item Records with zero-valued opening prices were removed as these entries are most likely data entry errors, and there are only a few of them so removing them will not introduce any major bias into the dataset.
\end{itemize}
...
The training setup was designed to ensure stable learning, prevent overfitting, and allow for fair comparison of the three RNN model architectures across five different regression time series datasets.
...
A hyperparameter grid was used to tune the hyperparameters of each of the models. The grid included the following parameters:
...
\subsection{\textbf{Training and Evaluation Utilities}}

To support the training and assessment of the RNNs, a set of utility functions was implemented to handle model training and evaluation across datasets and model architectures.

\subsubsection{\textbf{Epoch Training and Validation}}

Helpers were implemented to handle a single training epoch and a validation epoch. These utilities encapsulated the forward pass, loss computation, backpropagation, gradient clipping, and optimizer updates.

\subsubsection{\textbf{Prediction}}

A prediction utility function was implemented to generate prediction outputs for datasets. This function supported inverse transformation of scaled predictions back to the original space, which enabled meaningful interpretation of results.

\subsubsection{\textbf{Cross-Validation}}

Utilities were implemented to facilitate rolling-window cross-validation, which take care fo splitting training data into contiguous training and validation blocks, re-initialization of models for each fold to prevent information leakage, and recording performance metrics on each fold for later evaluation.

\subsubsection{\textbf{Performance Metrics}}

Multiple evaluation metrics were implemented to evaluate model performance on regression tasks:

\begin{itemize}
    \item \textbf{Root Mean Squared Error (RMSE)}: Root of average squared deviation. Penalizes large discrepancies.
    \item \textbf{Mean Absolute Error (MAE)}: Average absolute deviation.
    \item \textbf{Coefficient of Determination ($R^2$)}: Quantifies variance in observed data explained by the model. 
\end{itemize}

\subsubsection{\textbf{Hyperparameter Search}}

Utilities were implemented to perform grid search to find the optimal hyperparameter configuration for each model. These utilities built models with different hyperparameter combinations, trained and validated them using cross-validation, and selected the hyperparameter set resulting in the best average validation performance.
